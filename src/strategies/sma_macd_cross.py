...
def _slope_color(curr: float, prev: float, older: float) -> str:
    ...
    Tính màu slope từ 3 điểm liên tiếp — giống rule chart.js slopeColor:
      ...
      curr == prev            → yellow
    Trả về "yellow" nếu bất kỳ giá trị nào là NaN (chưa đủ warmup).
    """
    import math as _m
...
class SmaMacdCrossStrategy(BaseStrategy):
    ...
    requires_one_shot_check = True  # Chỉ 1 lệnh mỗi phase Signal bullish/bearish

    @classmethod
    def get_required_lookback(cls, parameters: dict) -> int:
        """Lookback phải đủ cho MACD signal_length (có thể lên tới 500)."""
