...
class SmaMacdCrossV2Strategy(BaseStrategy):
    ...
    Tham số mới/thay đổi so với V1:
    - bb_length (int): Chu kỳ MA cơ sở, mặc định 150 (V1: 50)
    - use_trend_filter (bool): Bật/tắt bộ lọc trend, mặc định True

    Tham số giữ nguyên:
    - fast_len, slow_len, len_c, factor: SMA params
    - macd_fast, macd_slow, macd_signal_length, macd_src, macd_sig_type: MACD params
    """

    def __init__(self, config: dict):
        super().__init__(config)
        self.name = "sma_macd_cross_v2"
        # SMA params
